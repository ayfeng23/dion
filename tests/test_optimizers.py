...
class TestNorDion2:
    ...
    def test_variance_neuron_dtype_fp32(self):
        """variance_neuron buffer must be float32 regardless of param dtype."""
        from dion import NorDion2
        for param_dtype in [torch.float32, torch.bfloat16, torch.float16]:
            ...
            opt.step()
            state = opt.state[params[0]]
            assert state["variance_neuron"].dtype == torch.float32, (
                f"variance_neuron should be fp32 but got {state['variance_neuron'].dtype} "
                f"for param dtype {param_dtype}"
            )
