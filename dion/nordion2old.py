...
class NorDion2Old(DistributedOrthoBase):
    """
    Distributed NorDion2Old optimizer for PyTorch FSDP2. Also compatible with DDP.

    Args:
        params: Parameters for the optimizer.
        distributed_mesh: DeviceMesh or ProcessGroup for distributed training.
            Use DeviceMesh for FSDP2 and ProcessGroup for DistributedDataParallel.
        lr: Base learning rate. For NorDion2Old, this will be scaled based on the matrix dimensions.
            For element-wise update rules, this is the actual learning rate and no additional scaling is done.
        fraction: Fraction of submatrix to orthogonalize per update (0 < fraction <= 1).
        mu: Momentum factor for NorDion2Old algorithm.
        muon_beta2: Second beta parameter for NorDion2Old algorithm's adaptive updates.
    ...
    NorDion2Old optimizer: https://arxiv.org/abs/2510.05491
    """
    ...
    def __init__(
        ...
        super().__init__(
            params, distributed_mesh, "nordion2old", defaults,
    ...
    def _get_shard_info(self, param: Tensor, group: dict):
        ...
        if is_matrix_sharded and sharded_tensor_dim == param.ndim - 1:
            raise NotImplementedError(
                "NorDion2Old currently does not support parameters sharded along the last dimension. "
    ...
    def _create_ortho_tasks(
        ...
        """
        Mega-batched NorDion2Old task creation: groups ALL same-shape parameters
        ...
        for group in param_groups:
            ...
            ), "NorDion2Old optimizer only supports matrix parameters."
            ...
            for (_shape, _sharding, _dtype), items in shape_groups.items():
                ...
                yield AsyncTask(
                    nordion2old_update_megabatch_async(
                        X=params,
                        G=gradients,
                        M=momentums,
                        V=variances_neuron,
...
def nordion2old_update_megabatch_async(
    X: List[Tensor],
    G: List[Tensor],
    M: List[Tensor],
    V: List[Tensor],
    ...
    """
    Mega-batched NorDion2Old update: processes ALL same-shape parameters in one
    ...
    U, U_selected, indices_list = nordion2old_pre_orthogonalize(
        G=to_local(G),
        M=to_local(M),
        V=to_local(V),
        fraction=fraction,
...
def nordion2old_normalization(
    ...
    """
    NorDion2Old normalization step.
    ...
    neuron_norms = [g_sq.mean(dim=-1, keepdim=True) for g_sq in G_sq]  # Shape: [*, rows, 1]
    ...
    U = torch._foreach_div(M, denom)  # list of m / denom
    U = [u.to(dtype=torch.bfloat16) for u in U]  # Convert to bfloat16 for orthogonalization
    return U

def nordion2old_pre_orthogonalize(
    G: List[Tensor],
    M: List[Tensor],
    V: List[Tensor],
    fraction: float,
    ...
    U = nordion2old_normalization(G=G, M=M, V=V, muon_beta2=muon_beta2)

    U_stacked = torch.stack(U, dim=0)

    if k_sel == "topk":
        slice_norms = U_stacked.norm(p=1, dim=-1)
        scores = slice_norms
    elif k_sel == "topk_M":
        ...
        num_rows = U_stacked.size(1)
        scores = torch.rand(batch_size, num_rows, device=U_stacked.device)
    else:
